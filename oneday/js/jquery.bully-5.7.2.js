...
(function($, window, document, undefined) {
	...
	if ( oneday_Bully.disable_mobile ) {
	...
	(function update() {
		if (frameRendered !== true) {
			...
			$.each(oneday_Bully.sections, function(id, arg) {
				var element = $("#" + id);
			...
			if (
				lastItemId &&
				typeof oneday_Bully.sections[lastItemId] !== "undefined"
			) {
				if (oneday_Bully.sections[lastItemId].inverse) {
	...
	function Bully(element, options) {
		...
		if (oneday_Bully.enable_label) {
			if (id && typeof oneday_Bully.sections[id] !== "undefined") {
				label = oneday_Bully.sections[id].title;
			}
			if (label) {
				$bullet.append(
...
jQuery(document).ready(function($) {
	$.each(oneday_Bully.sections, function(id, args) {
		if (args.enable) {
			$("#" + id).bully({
